  ...
  if ( (OS_IsRoot() AND isset( $_GET["edit"]) AND is_numeric($_GET["edit"]) ) OR isset($_GET["add"])  ) {
   ...
   //UPDATE
    if ( isset($_POST["edit_user"]) ) {
	  ...
	  if ( isset($_GET["add"]) ) $sql = "INSERT INTO ".OSDB_USERS."(user_name, alias_id, user_email, user_password, password_hash, user_joined, bnet_username, user_bnet, user_realm, user_clan, blacklisted, user_level_expire, user_level, user_ip, confirm) VALUES('".$name."', '".$alias_id."', '".$email."', '".$password_db."', '".$hash."', '".time()."', '".$bnet."', '".$user_bnet."', '".$user_realm."', '".$user_clan."', '".$blacklisted."', '".$user_level_expire."', ".$level.", '".$_SERVER['REMOTE_ADDR']."', 1)";
